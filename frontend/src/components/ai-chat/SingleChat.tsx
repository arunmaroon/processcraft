...
const SingleChat: React.FC<SingleChatProps> = ({ agent }) => {
  ...
  useEffect(() => {
    ...
    const greeting = agent.speakingPatterns.greetings[
      Math.floor(Math.random() * agent.speakingPatterns.greetings.length)
    ];
  ...
  const generateHumanResponse = async (userMessage: string): Promise<Message> => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const agentResponse = await generateHumanResponse(inputValue);
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {agent.background.occupation} • {agent.background.age} years • {agent.background.location}
            </p>
          </div>
