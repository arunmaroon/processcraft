...
interface Message {
  id: string;
  type: 'user' | 'agent1' | 'agent2' | 'agent3' | 'system';
  content: string;
  timestamp: Date;
  emotion?: string;
  confidence?: number;
...
const TripleChat: React.FC<TripleChatProps> = ({ agents }) => {
  const [messages, setMessages] = useState<Message[]>([]);
  const [inputValue, setInputValue] = useState('');
  const [isTyping, setIsTyping] = useState(false);
  const [isGenerating, setIsGenerating] = useState(false);
  const [activeAgent, setActiveAgent] = useState<'agent1' | 'agent2' | 'agent3'>('agent1');
  const [conversationMode, setConversationMode] = useState<'turn-based' | 'group'>('turn-based');
  ...
  useEffect(() => {
    ...
    const greeting3 = agent3.speakingPatterns.greetings[
      Math.floor(Math.random() * agent3.speakingPatterns.greetings.length)
    ];
    ...
    const initialMessages: Message[] = [
      ...
      },
      {
        ...
        type: 'system',
        content: `${agent1.name}, ${agent2.name}, and ${agent3.name} are ready for a group discussion! What would you like to explore?`,
        timestamp: new Date()
      }
    ];
  ...
  const generateHumanResponse = async (userMessage: string, agent: HumanAgent, agentType: 'agent1' | 'agent2' | 'agent3'): Promise<Message> => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      if (conversationMode === 'turn-based') {
        // Generate response from active agent
        const activeAgentData = activeAgent === 'agent1' ? agent1 : activeAgent === 'agent2' ? agent2 : agent3;
        const agentResponse = await generateHumanResponse(inputValue, activeAgentData, activeAgent);
        setMessages(prev => [...prev, agentResponse]);
        
        // Switch to next agent for next response
        if (activeAgent === 'agent1') setActiveAgent('agent2');
        else if (activeAgent === 'agent2') setActiveAgent('agent3');
        else setActiveAgent('agent1');
      } else {
        // Group mode - generate responses from all agents
        const responses = await Promise.all([
          generateHumanResponse(inputValue, agent1, 'agent1'),
          generateHumanResponse(inputValue, agent2, 'agent2'),
          generateHumanResponse(inputValue, agent3, 'agent3')
        ]);
  ...
  const getAgentInfo = (messageType: string) => {
    ...
    if (messageType === 'agent3') return { agent: agent3, color: 'bg-purple-100' };
    return null;
  };

  const getNextAgent = () => {
    if (activeAgent === 'agent1') return agent2;
    if (activeAgent === 'agent2') return agent3;
    return agent1;
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-lg font-semibold text-gray-900">Triple Chat</h3>
            </div>
            <div className="flex items-center space-x-2">
              <div className="text-lg">{agent1.avatar}</div>
              <div className="text-lg">{agent2.avatar}</div>
              <div className="text-lg">{agent3.avatar}</div>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-600">{agent1.name}</span>
              <span className="text-gray-400">+</span>
              <span className="text-sm text-gray-600">{agent2.name}</span>
              <span className="text-gray-400">+</span>
              <span className="text-sm text-gray-600">{agent3.name}</span>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-2">
              <div className="text-xs text-gray-500">
                {conversationMode === 'turn-based' ? `Next: ${getNextAgent().name}` : 'All agents'}
              </div>
            ...
            <div className="bg-gray-100 text-gray-900 px-4 py-2 rounded-lg">
              <div className="flex items-center space-x-2">
                <div className="text-lg">
                  {conversationMode === 'turn-based' 
                    ? (activeAgent === 'agent1' ? agent1.avatar : activeAgent === 'agent2' ? agent2.avatar : agent3.avatar)
                    : '🤖'
                  }
                </div>
                <div className="text-xs font-medium text-gray-600 mb-1">
                  {conversationMode === 'turn-based' 
                    ? (activeAgent === 'agent1' ? agent1.name : activeAgent === 'agent2' ? agent2.name : agent3.name)
                    : 'Agents thinking...'
                  }
                </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="flex-1 relative">
            <input
              ...
              placeholder={
                conversationMode === 'turn-based' 
                  ? `Message ${activeAgent === 'agent1' ? agent1.name : activeAgent === 'agent2' ? agent2.name : agent3.name}...`
                  : 'Message all agents...'
              }
