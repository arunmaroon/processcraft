...
router.post('/documents/process', upload.array('files'), async (req, res) => {
  try {
...
// Multimodal Analysis
router.post('/analyze/multimodal', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
...
// Consistency Scoring
router.post('/analyze/consistency', async (req, res) => {
  try {
    const { conversationHistory } = req.body;
...
// Memory Management
router.get('/memory/agent/:agentId', async (req, res) => {
  try {
...
// Alerts Management - Temporarily disabled
